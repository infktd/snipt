...
func (r ResultList) renderSelectedRow(item ResultItem) string {
	...
	rightTopPad := lipgloss.NewStyle().Width(rightWidth).Background(ColorBgSelected).Render("")
	...
	for len(leftLines) < maxH {
		leftLines = append(leftLines,
			lipgloss.NewStyle().Width(leftWidth).Background(ColorBgSelected).Render(""))
	}

	// Pad right if needed.
	for len(rightLines) < maxH {
		rightLines = append(rightLines,
			lipgloss.NewStyle().Width(rightWidth).Background(ColorBgSelected).Render(""))
	}
	...
	barStyle := lipgloss.NewStyle().Foreground(ColorPink).Background(ColorBgSelected)
	barAfter := lipgloss.NewStyle().Width(1).Background(ColorBgSelected) // breathing room
	gapStyle := lipgloss.NewStyle().Width(1).Background(ColorBgSelected)
...
func (r ResultList) buildSelectedLeftLines(item ResultItem, width int) []string {
	...
	if sn.Pinned {
		titleParts = append(titleParts,
			lipgloss.NewStyle().Foreground(ColorPink).Background(ColorBgSelected).Render("✦"))
	} else {
		titleParts = append(titleParts,
			lipgloss.NewStyle().Background(ColorBgSelected).Render(" "))
	}

	title := renderFuzzyTitleWithBg(sn.Title, item.FuzzyResult.Indices, true, ColorBgSelected)
	...
	if lipgloss.Width(titleContent) > width {
		...
		titleContent += lipgloss.NewStyle().Foreground(ColorTextDim).Background(ColorBgSelected).Render("…")
	}
	...
	if sn.Language != "" {
		metaParts = append(metaParts, renderLangBadge(sn.Language, ColorBgSelected))
	}

	if len(sn.Tags) > 0 {
		used := 2 + lipgloss.Width(strings.Join(metaParts, " ")) // indent + lang badge
		for _, tag := range sn.Tags {
			rendered := renderTagBadge(tag, ColorTextSub, ColorBgSelected)
	...
	metaContent := lipgloss.NewStyle().Background(ColorBgSelected).Render("  ") +
		strings.Join(metaParts, lipgloss.NewStyle().Background(ColorBgSelected).Render(" "))

	if lipgloss.Width(metaContent) > width {
		metaContent = lipgloss.NewStyle().MaxWidth(width - 1).Render(metaContent)
		metaContent += lipgloss.NewStyle().Foreground(ColorTextDim).Background(ColorBgSelected).Render("…")
	}
...
func (r ResultList) renderPreviewBox(sn model.Snippet, width int) string {
	if sn.Content == "" {
		return lipgloss.NewStyle().Width(width).Background(ColorBgSelected).Render("")
	}
	...
	for _, line := range lines[:maxLines] {
		...
		if truncated {
			highlighted += lipgloss.NewStyle().Foreground(ColorTextMuted).Background(ColorBgSurface).Render("…")
		}
	...
	if len(lines) > maxLines {
		moreStyle := lipgloss.NewStyle().Foreground(ColorTextMuted).Background(ColorBgSurface)
	...
	previewStyle := lipgloss.NewStyle().
		...
		BorderBackground(ColorBgSelected).
...
func (r ResultList) renderSingleRow(item ResultItem, selected bool) string {
	...
	if sn.Pinned {
		pinStyle := lipgloss.NewStyle().Foreground(ColorPink).Background(rowBg)
		parts = append(parts, pinStyle.Render("✦"))
	} else {
		parts = append(parts, lipgloss.NewStyle().Background(rowBg).Render(" "))
	}

	title := renderFuzzyTitleWithBg(sn.Title, item.FuzzyResult.Indices, selected, rowBg)
	parts = append(parts, title)

	if sn.Language != "" {
		parts = append(parts, renderLangBadge(sn.Language, rowBg))
	}

	if len(sn.Tags) > 0 {
		tagColor := ColorTextDim
		if selected {
			tagColor = ColorTextSub
		}
		...
		for _, tag := range sn.Tags {
			rendered := renderTagBadge(tag, tagColor, rowBg)
			needed := lipgloss.Width(rendered) + 1
			if used+needed > width {
				break
			}
...
func syntaxHighlightLine(line string, language string) string {
	...
	if commentIdx == 0 {
		...
		return lipgloss.NewStyle().Foreground(ColorTextMuted).Background(ColorBgSurface).Render(line)
	}
	...
	if commentPart != "" {
		highlighted += lipgloss.NewStyle().Foreground(ColorTextMuted).Background(ColorBgSurface).Render(commentPart)
	}
...
func highlightTokens(code string, language string) string {
	...
	kwStyle := lipgloss.NewStyle().Foreground(ColorMauve).Bold(true).Background(ColorBgSurface)
	strStyle := lipgloss.NewStyle().Foreground(ColorGreen).Background(ColorBgSurface)
	numStyle := lipgloss.NewStyle().Foreground(ColorPeach).Background(ColorBgSurface)
	defaultStyle := lipgloss.NewStyle().Foreground(ColorTextSub).Background(ColorBgSurface)
