...
var (
	...
	formBorderFocusedStyle = lipgloss.NewStyle().
				Border(lipgloss.RoundedBorder()).
				BorderForeground(common.ColorBorderFocus).
				Padding(1, 2)
...
func newStyledInput(placeholder string, charLimit, width int) textinput.Model {
	...
	ti.SetWidth(width)
	s := ti.Styles()
	s.Focused.Prompt = lipgloss.NewStyle().Foreground(common.ColorMauve)
	s.Focused.Text = lipgloss.NewStyle().Foreground(common.ColorText)
	s.Focused.Placeholder = lipgloss.NewStyle().Foreground(common.ColorTextMuted)
	s.Blurred.Prompt = lipgloss.NewStyle().Foreground(common.ColorTextDim)
	s.Blurred.Text = lipgloss.NewStyle().Foreground(common.ColorTextDim)
	s.Blurred.Placeholder = lipgloss.NewStyle().Foreground(common.ColorTextMuted)
	ti.SetStyles(s)
	return ti
}
